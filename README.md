...
> Grad-CAM 비교 시, baseline GDN과 CBAM 모델의 target layer가 달라 비교가 무효해지는 문제를 발견 → CBAM 모델은 spatial attention map을 직접 시각화하는 방식으로 전환해 공정한 비교를 확보. 아래 두 그림은 결절 크기(작은→큰) 순으로 정렬한 동일 케이스에 대해 ConvNeXt → DualConvNeXt → GDN(+CBAM)을 나란히 비교한 결과.

**TN 케이스 (양성 결절을 양성으로 분류)**

![Grad-CAM TN comparison](assets/figures/gradcam_TN_comparison.png)

- Baseline: feature가 약해 노이즈성 신호에 의존하거나 결절 중심부에만 attention 집중
- CBAM 적용 후: 결절 자체 특징이 약할 때 주변 맥락(혈관, 흉막 구조)으로 attention을 확장해 판단 → "결절 자체의 특징이 없으니 맥락으로 판단"

**TP 케이스 (악성 결절을 악성으로 분류)**

![Grad-CAM TP comparison](assets/figures/gradcam_TP_comparison.png)

- Baseline 대비 hotspot이 결절 주변으로 확장됨
- CBAM 적용 후: attention이 중심점에서 경계 구조물로 재분배되어 spiculation(침상 돌기), lobulation(분엽) 등 악성 형태 특징을 추가로 포착 → 임상적으로 더 올바른 판단 근거
